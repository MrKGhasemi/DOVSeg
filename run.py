...
def process_image(image_path, models, args):
    ...
    if args.mode == "blip":
        ...
        noun_phrases, text_prompt = class_generators.get_classes_llm(
            image_path, models, args.api_key, args.base_url
        )
    ...
    CLASS_COLORS = [(np.random.randint(0, 255), np.random.randint(0, 255), np.random.randint(0, 255)) for _ in noun_phrases]
    ...
    transformed_boxes = models['sam_predictor'].transform.apply_boxes_torch(boxes_for_sam, image_rgb.shape[:2]) 
    ...
    if configs.SAVE_FIGURES:
            ...
            utils.save_visualizations(instance_map, semantic_map, output_prefix)
