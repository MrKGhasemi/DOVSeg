...
def parse_and_clean_nouns(raw_text, nlp):
    ...
    for chunk in doc.noun_chunks:
        ...
        if head.isalpha() and head not in {"a", "an", "the", "color", "thing", "someone", "something", "it", "this", "that", "which", "what", "background"}:
...
def encode_image(image_input):
    """
    Encodes an image in Base64 for API calls.
    ...
    # Case 2: Input is a PIL Image (e.g., a crop)
...
def save_visualizations(output_image_with_labels, semantic_color_map, output_path_prefix,  mode):
    ...
    cv2.imwrite(f"{output_path_prefix}_instance_{mode}.jpg", instance_map_bgr)
    cv2.imwrite(f"{output_path_prefix}_semantic_{mode}.png", semantic_map_bgr)
